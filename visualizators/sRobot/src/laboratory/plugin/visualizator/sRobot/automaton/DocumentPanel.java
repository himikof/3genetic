package laboratory.plugin.visualizator.sRobot.automaton;
...
//        }
    ...
    private Graph createGraph(Automaton a, int width, int height){
        ...
        for(int i = 0;i < n;i++){
            ...
            g.addNode(new EllipseNode(i, a.getStateString(i),
        ...
        for(int i = 0;i < a.getNumberStates();i++){
            for (int e = 0; e < 2; ++e) {
                ...
                if((t != null) && (t.getEndState() != -1)){
                    String name = eventNames[e] + ", " + t.getAction();
                    if(t.getEndState() == i){
                        g.addEdge(g.getNode(i), g.getNode(i), new Loop(name));
                    } else{
                        g.addEdge(g.getNode(i), g.getNode(t.getEndState()), new Edge(name));
                    }
                }                
            }
        }
