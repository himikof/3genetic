...
//        }
    ...
    public DocumentPanel(boolean edit, Automaton a, AutomatonRunner m, int width, int height){
    ...
    private Graph createGraph(Automaton a, int width, int height){
        ...
        for(int i = 0;i < n;i++){
            ...
            g.addNode(new EllipseNode(i, a.getStateString(i), x, y, i == a.getInitialState()));
            alpha += dalpha;
        }
        for(int i = 0;i < a.getNumberStates();i++){
            ...
            if((t != null) && (t.getEndState() != -1)){
                if(t.getEndState() == i){
                    g.addEdge(g.getNode(i), g.getNode(i), new Loop("T, " + t.toString()));
                } else{
                    g.addEdge(g.getNode(i), g.getNode(t.getEndState()), new Edge("T, " + t.toString()));
                }
            }
            t = a.getTransition(i, 0);
            if((t != null) && (t.getEndState() != -1)){
                if(t.getEndState() == i){
                    g.addEdge(g.getNode(i), g.getNode(i), new Loop("F, " + t.toString()));
                } else{
                    g.addEdge(g.getNode(i), g.getNode(t.getEndState()), new Edge("F, " + t.toString()));
                }
            }
        }
        return g;
